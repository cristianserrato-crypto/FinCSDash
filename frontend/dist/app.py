...
try:
    ...
    print("AVISO: fpdf no instalado. Usando stub para evitar crash.")
    FPDF = None
...
if os.path.exists(env_file):
    ...
    print("DEBUG: .env cargado correctamente.")
else:
...
CORS(app, resources={
    r"/*": {
        "origins": "*",  # Permitir todos los orígenes en desarrollo/servidor personal
...
if not JWT_SECRET_KEY:
    print("ERROR CRÍTICO: JWT_SECRET_KEY no encontrada en el entorno.")
    JWT_SECRET_KEY = "cambiame-en-produccion-muy-importante" # Valor temporal para evitar crash

if not GOOGLE_CLIENT_ID:
    print("ERROR CRÍTICO: GOOGLE_CLIENT_ID no encontrada en el entorno.")
    GOOGLE_CLIENT_ID = "google-id-dummy" # Valor temporal
...
try:
    ...
    print(f"ADVERTENCIA: No se pudieron crear las tablas al iniciar: {e}")
...
# Manejador global de errores para asegurar que siempre responda JSON
...
def login():
    ...
    try:
        ...
        return jsonify({"message": "Error interno del servidor al conectar a la base de datos"}), 500
...
if __name__ == "__main__":
    try:
        ...
        if e.errno == 98:
            print("ERROR: El puerto 5000 ya está en uso. Ejecuta 'sudo fuser -k 5000/tcp' para liberarlo.")
        else:
            raise e
