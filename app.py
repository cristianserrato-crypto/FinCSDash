...
def update_photo():
    ...
    foto_url = foto # Por defecto, si no hay S3, guardamos el base64 (aunque no es ideal para SQLite)

    # Verificamos si tenemos al menos el bucket y la región configurados
    if s3_bucket and s3_region:
        try:
            # Si hay credenciales explícitas, las usamos. Si no, boto3 buscará el Rol IAM del EC2 automáticamente.
            ...
            # Subir a S3
            s3.put_object(Bucket=s3_bucket, Key=filename, Body=file_content, ContentType=f"image/{ext}")
            
            foto_url = f"https://{s3_bucket}.s3.{s3_region}.amazonaws.com/{filename}"
        except Exception as e:
...
def get_movements():
    ...
    query_ingresos = "SELECT id, monto, fecha, categoria FROM ingresos WHERE usuario_id = %s"
    query_gastos = "SELECT id, tipo, monto, fecha, es_recurrente FROM gastos WHERE usuario_id = %s"
    params = [usuario_id]
...
def export_pdf():
    ...
    query = "SELECT tipo, monto, fecha FROM gastos WHERE usuario_id = %s"
    params = [usuario_id]
...
if __name__ == "__main__":
    ...
    # host="0.0.0.0" permite que el servidor sea accesible desde la IP pública de AWS
